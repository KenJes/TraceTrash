...
export default function AjustesScreen() {
  ...
  const handleSaveField = async (field: 'nombre' | 'direccion', value: string) => {
    if (!value.trim()) {
      Alert.alert('Error', `El ${field === 'nombre' ? 'nombre' : 'dirección'} no puede estar vacío`);
      return;
    }
    try {
      if (user?.uid) {
    ...
    <ThemedView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          <View style={{ marginBottom: 16 }}>
            <View style={[styles.spaceBetween, { marginBottom: 8 }]}>
              ...
              <TouchableOpacity onPress={() => { setTempDireccion(user?.direccion || ''); setEditingAddress(true); }}>
                <Ionicons name="pencil" size={18} color="#43A047" />
              </TouchableOpacity>
            </View>
        ...
        <View style={styles.card}>
          ...
          <ThemedText style={styles.bodyText}>Tema: {isDarkMode ? 'Oscuro' : 'Claro'}</ThemedText>
        </View>
