...
export default function ReportarScreen() {
	...
	const opcionesIncidencia = isConductor ? opcionesConductor : opcionesResidente;
	...
	const usarDireccionUsuario = () => {
		if (user?.direccion) {
			...
			Alert.alert('Sin dirección', 'No tienes una dirección registrada en tu perfil');
		}
	};
	...
	*/
	...
	const handleEnviar = async () => {
		...
		try {
			...
			setIsLoading(false);
			setLoadingMessage('');
			Alert.alert('Error', error.message || 'No se pudo enviar el reporte. Verifica tu conexión a internet.');
		}
	};
	...
	return (
		<ThemedView style={styles.container}>
			...
			>
				<View style={{
					...
					<View style={{
						...
						<ThemedText style={{ marginTop: 16, fontSize: 16, fontWeight: '600' }}>
							{loadingMessage}
						</ThemedText>
					</View>
				</View>
			...
			<View style={styles.card}>
				...
				<View style={styles.card}>
					<ThemedText style={styles.inputLabel}>Descripción del problema</ThemedText>
					<TextInput
						style={styles.input}
				...
				<View style={styles.card}>
					...
					{ubicacion && (
						<View style={{
							...
							<ThemedText style={{ flex: 1, fontSize: 14 }}>{ubicacion}</ThemedText>
							<TouchableOpacity onPress={() => setUbicacion(null)}>
								<ThemedText style={{ color: '#E53935', fontWeight: '600' }}>Borrar</ThemedText>
							</TouchableOpacity>
						</View>
					)}
					...
					{mostrarInputUbicacion ? (
						<View>
							...
							<View style={{ flexDirection: 'row', gap: 8 }}>
								<TouchableOpacity
									style={[styles.boton, { flex: 1, backgroundColor: '#4CAF50' }]}
									onPress={guardarUbicacionManual}
								>
									<ThemedText style={styles.botonTexto}>Guardar</ThemedText>
								</TouchableOpacity>
								<TouchableOpacity
									style={[styles.boton, { flex: 1, backgroundColor: '#9E9E9E' }]}
									onPress={() => {
										setMostrarInputUbicacion(false);
										setUbicacionManual('');
									}}
				...
				<TouchableOpacity 
					style={[styles.boton, (isLoading || !descripcion.trim()) && { opacity: 0.6 }]} 
